  ...
  for (const product of products) {
    ...
    if (product.companyName.name) {
      product.companyName.name =
        ...
        product.companyName.name.charAt(0).toUpperCase() + product.companyName.name.slice(1).toLowerCase();
    } else {
...
async function updateCategoryAndBrands() {
  ...
  for (const product of products) {
    ...
    if (product.companyName.name) {
      const brandFound = await Brand.findOne({ name: product.companyName.name });
      if (!brandFound) {
  ...
  for (const category of categories) {
    if (brands.find((brand) => brand.name.toLowerCase() === category.name.toLowerCase())) {
