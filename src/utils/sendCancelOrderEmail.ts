...
export const sendCancelOrderEmail = async (
  ...
  const mailOptions = {
    from: `XpressBeauty Order Cancelled <${import.meta.env.VITE_EMAIL ?? ""}>`,
    to: email.trim(),
    ...
    <div style="max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ccc; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        ...
        <h1 style="text-align: center; margin-bottom: 20px;">Order Cancelled</h1>
        <p style="text-align: center; margin-bottom: 20px;">Order Number: <span style="font-weight: bold;"> ${orderNumber} </span> </p>
        <div style="margin-bottom: 20px;">
        <p>Dear ${name},</p>
        <p>We are sorry to inform you that your order has been cancelled.
        That may be due to a payment issue or 
        stock availability be assured that you will not be charged for this order. If you have any questions, please contact us at
        <a href="mailto:
