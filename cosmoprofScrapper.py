...
def get_products_ids():
    categoriesArray = [
        {'prefix': "hair-color", 'name': "Hair Colour"},
        {"prefix": "hair-care", "name": "Hair Care"},
        {"prefix": "skin-and-body", "name": "Skin & Body"},
        {"prefix": "nails", "name": "Nails"},
    ]
    ...
    for category in categoriesArray:
        ...
        while True:
            try:
                url = f'''https://www.cosmoprofbeauty.ca/on/demandware.store/Sites-CosmoProf-CA-Site/default/Search-UpdateGrid?cgid={category['prefix']}&start={page}&sz={limit}'''
                driver.get(url)
                while has_captcha(driver):
                    handle_captcha(driver, url)
                soup = BeautifulSoup(driver.page_source, 'html.parser')
                productsDivs = soup.find_all('div', 'product-tile')
                if len(productsDivs) == 0:
                    break
                for productValue in productsDivs:
                    productJson = productValue['data-ga4tile']
                    validJson = productJson.replace("'", '"')
                    productData = json.loads(validJson)
                    finalJson = productData['ecommerce']['items'][0]
                    try:
                        productsIds.append({
                            'cosmoprof_id': finalJson['item_id'],
                            'category': {
                                'main': category['name'],
                                'name': finalJson['item_category']
                            },
                            'companyName': {
                                'name': finalJson['item_brand']
                            }
                        })
    ...
    for productId in productsIds:
        try:
            ...
            product['product_image'] = upload_image(
                imageUrl, f'''{product['product_name'].replace(' ', '-')}_image''')
            if 'longDescription' in parsed_json['product']:
            ...
            if parsed_json['product']['productType'] == 'master':
                ...
                for variationCosmo in parsed_json['product']['variationAttributes'][0]['values']:
                    ...
                    variation['variation_image'] = upload_image(
                        image, f'''{variationCosmo['displayValue'].replace(' ', '-')}_{product["product_name"].replace(' ', '-')}_image''')
